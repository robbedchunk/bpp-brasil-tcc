...
async def media_handling():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        for img in result.media["images"][:5]:
            print(
                f"Image URL: {img['src']}, Alt: {img['alt']}, Score: {img['score']}")
...
async def capture_and_save_screenshot(url: str, output_path: str):
    browser_config = BrowserConfig(headless=True)
    crawler_config = CrawlerRunConfig(
        cache_mode=CacheMode.BYPASS, screenshot=True)
...
async def crawl_dynamic_content_pages_method_1():
    ...
    async def on_execution_started(page, **kwargs):
        ...
        try:
            ...
            print(
                f"Warning: New content didn't appear after JavaScript execution: {e}")
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        print(
            f"Successfully crawled {len(all_commits)} commits across 3 pages")
...
async def cosine_similarity_extraction():
    ...
    crawl_config = CrawlerRunConfig(
        ...
        extraction_strategy=CosineStrategy(
            ...
            # Keywords to filter the content semantically using embeddings
...
async def crawl_with_user_simulation():
    browser_config = BrowserConfig(
        ...
        user_agent_generator_config={
            "device_type": "mobile", "os_type": "android"},
    )
