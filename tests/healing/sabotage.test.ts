...
describe("isolated staging sabotage", () => {
  it("heals a broken active selector through fake generation and trusted 30-sample validation", async () => {
    ...
    const evidenceDomain = "shop.test";
    database.prepare(
      `INSERT INTO retailers
         (id, name, base_url, cep, domains_json, active)
       VALUES ('sabotage', 'Sabotage fixture', ?, '01310-100', ?, 1)`,
    ).run(server.origin, JSON.stringify([domain, evidenceDomain]));
    const broken = validStrategy(domain);
    broken.allowedDomains.push(evidenceDomain);
    ...
    database.prepare(`
      ...
      WHERE id = 'sabotage-extraction-v1'
    `).run();
    ...
    for (let index = 0; index < 30; index += 1) {
      insertProduct.run(
        `product-${index}`,
        `https://${evidenceDomain}/products/${index}`,
        String(index),
        `Fixture ${index}`,
      );
    }
    ...
    const execute = (strategy: Parameters<typeof executeDom>[0], ref: Parameters<typeof executeDom>[1]) =>
      executeDom(strategy as DomExtractionStrategy, {
        ...ref,
        canonicalUrl: `${server.origin}${new URL(ref.canonicalUrl).pathname}`,
      }, { browser });
    ...
    const generator: StrategyGenerator = {
      async generate(_request: GenerationRequest) {
        ...
        return {
          ...
          strategy: {
            ...validStrategy(domain),
            allowedDomains: [domain, evidenceDomain],
          },
    ...
    const worker = await healPendingEvents({
      ...
      generator,
      execute,
      validateCandidate: async (strategy, refs, context) =>
        signedCandidateReport(strategy, refs, context, 1),
      now: () => new Date("2026-07-10T01:05:00.000Z"),
    });
